...
4. 配置集中在 `app/core/config.py`（pydantic-settings）；`.env.example` 已预留 `DATABASE_URL`、`WECHAT_APPID`、`WECHAT_SECRET`、`JWT_SECRET`、`JWT_EXPIRE_DAYS`。
...
1. 本地开发：按 `.env` 的 `DATABASE_URL` 连接本地 MySQL；库需手动先建（`CREATE DATABASE ai_quiz CHARACTER SET utf8mb4;`），表由启动时自动创建。
2. 自动化测试：测试配置把 `DATABASE_URL` 覆盖为 SQLite（内存库），ORM 层保持方言中立（不写 MySQL 专属 SQL，upsert 用“先 SELECT 再 INSERT”的通用写法而非 `ON DUPLICATE KEY UPDATE`），保证测试无需真实 MySQL。
...
| 配置项 | 说明 | 缺省行为 |
| --- | --- | --- |
| `DATABASE_URL` | SQLAlchemy 连接串（`mysql+pymysql://...`） | 无缺省，启动必须配置 |
...
1. **密钥管理**：`JWT_SECRET`、`WECHAT_SECRET`、`DATABASE_URL` 只存在于 `.env`（已在 `.gitignore`），文档与代码中不出现真实值。
...
数据库策略：测试环境用 SQLite 内存库覆盖 `DATABASE_URL`（§4.5），ORM 层保持方言中立；涉及唯一约束冲突的用例同样可在 SQLite 上验证。
